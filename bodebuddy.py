...
def transfer_function(choice, zeros, poles, s):
    ...
    if choice == 1:
        ...
        num = (s + zeros[0]) * (s + zeros[1])
        den = (s + poles[0]) * (s + poles[1])
...
def bode_analysis(choice, zeros, poles, start_freq, stop_freq):
    """
    Perform Bode plot analysis for a given transfer function and frequency range.
    ...
    zeros : list of float
        List of zero frequencies (rad/s) for the transfer function.
    poles : list of float
        List of pole frequencies (rad/s) for the transfer function.
    start_freq : float
        Start frequency (Hz) of the frequency range for analysis.
    stop_freq : float
        Stop frequency (Hz) of the frequency range for analysis.

    Returns
    -------
    magnitude_phase_list : list of tuple
        List of tuples containing frequency (Hz), magnitude, and phase (degrees) data.

    Examples
    --------
    >> bode_analysis(1, [1000], [2000], 1, 1e6)
    [(1.0, 0.0126, -2.86), (1.02329, 0.0129, -2.94), ... , (1000000.0, 0.6667, -135.0)]
    """
    magnitude_phase_list = calculate_magnitude_and_phase(choice, zeros, poles, start_freq, stop_freq)
    return magnitude_phase_list
...
def run_interactive():
    while True:
        ...
        magnitude_phase_list = bode_analysis(choice, zeros, poles, start_freq, stop_freq)
        display_table(magnitude_phase_list)
        plot_bode(magnitude_phase_list)

        user_input = input(
            "Do you want to input another case or exit? "
            "(Enter 'c' to continue or any key to exit): "
        ).lower()
        if user_input != 'c':
            break
